...
define [
  ...
  $.widget 'app.mapItem', $.app.loadableImage, 
    ...
    destroy: ->
      @element.off()
      $.app.loadableImage::destroy.apply @, arguments
      @element.remove()
    ...
    _create: ->
      ...
      $.app.loadableImage::_create.apply @, arguments
      ...
      @bindTo @options.model, 'destroy', => @element.remove()
    ...
    _onUpdate: (model, changes) ->
      ...
      @element.remove() if @options.model?.get('map')?.id isnt @options.map.options.mapId
